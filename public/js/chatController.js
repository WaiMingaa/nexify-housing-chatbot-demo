...
chatapp.controller('ChatController', function ($scope,$parse,GencaseService,$compile,$http,$sce,LangHandler,MessageBuilder,PipelineService,MessageConnector, TimeHandler,ConversationHandler,  Upload, $q) {
    ...
    /****** function associated with upload button ******/
    $scope.add = function () {
        $("#file").click();
    }
      ...
      $http({
          ...
          if (response.data.text == '') {
            MessageBuilder.build( PipelineService.buildUserJSON("******")).then(function(dialog){
              ...
              MessageBuilder.build( PipelineService.buildBotJSON( Promise.resolve({"text":"聽唔清楚請你講多次/Please repeat"})))
  ...
  ;
    ...
    //     }
        ...
        submitform = function(...l){
          ...
          sendByButton('表格已經傳送');
          // MessageBuilder.build( PipelineService.buildBotJSON(Promise.resolve({"text":"多謝你的資料，我們將儘快處理。"}))).then(function(dialog){
          //   _dialog.addDialog(dialog);
          //   responsiveVoice.speak(dialog, 'Chinese (Hong Kong) Female');
          //   })
...
chatapp.service('MessageBuilder',function(TimeHandler,$q){
  ...
  this.genBotMessage=function(word){
    return '<div class="row"> <div class="col-md-7"><li class=\"self-left\"><img style="width:70x;height:50px;" src="./img/chaticon.png" aria-hidden="true"></img><div class="bubble-left">' + '<p align=\"left\">' + word + '</p></div><span class="timetext-left">' +
  ...
  this.genUserMessage=function(word) {
    return '<div class="row"> <div class="col-md-7 col-md-offset-5"><li class=\"self-right\"><span class="timetext-right">' +
          TimeHandler.getCurrentTime() + '</span><div class="bubble-right">' + word + '</div><img style="width:70x;height:50px;" src="./img/usericon.png" aria-hidden="true"></img></li></div></div>';
  }
})
...
this.buildBotJSON =function(promise){
  var _getBotJson =function(text){
    var json = {
        side:'bot',
        content:'<div class="ui fluid segment">'+text+'</div>'
    }
    return json;
  }
      ...
      response.data.output.text.forEach(function(text){
        ...
        if(response.data.context.event){
          response.data.context.event.forEach(function(e){
            ...
            jlist.push(_getBotJson(response.text));
        }
      deferred.resolve(jlist);
  })
  return deferred.promise;
...
chatapp.service('ConversationHandler', function () {
    ...
    this.getNewReqPayload = function ($http) {
