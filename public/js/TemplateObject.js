var templateObject = (function(){
  ...
  return{
    ...
    function buildTemplate(json){
        ...
        break;
		case 'link':
		return _buildlink(content);
        }
    }
    function _buildButtonForm(content){
      var str =  '<div class="ui form">';
      content.question.forEach(function(q){
        str +="<div style=\" display:inline-block;padding:5px;\" class=\"inline grouped fields\">"
    ...
    content.items.forEach(function(item){
      if(item.type=="select"){
        ...
        str += "      <div  class=\"ui selection dropdown\">"
          str += '          <input type=\"hidden\"   id='+"\""+item.id+"\""+'  value="{{'+item.id+'}}" >'
       str += "          <i class=\"dropdown icon\"><\/i>";
       str += "          <div class=\"default text\" >"+item.name +" : {{"+item.id+"}}"+"<\/div>"
      str += "          <div class=\"menu\">";
    ...
    function _buildCardItem(content){
      var count =0;
      var str ='<div class="ui  tabular menu">';
        content.items.forEach(function(item){
          if(count==0)
       str +='<div class="active item" data-tab="'+item.tab+'" style="padding:5px">' +item.tab+'</div>'
       else
         str +='<div class="item" data-tab="'+item.tab+'" style="padding:5px">' +item.tab+'</div>'
       count+=1;
        })
        str+='</div>'
        var component =''
  ...
  var str =  '<div class="ui vertical buttons">';
    ...
    function _buildYesNoButton(content){
      ...
      content.buttons.forEach(function(button){
      if (count ==0)
            component+='<button  onClick="sendByButton('+"'"+button.value+"'"+')" class="ui   fluid button">'+button.name+'</button>';
            else{
              component+='<button  onClick="sendByButton('+"'"+button.value+"'"+')" class="ui fluid button" >'+button.name+'</button>';
            }
            count +=1
